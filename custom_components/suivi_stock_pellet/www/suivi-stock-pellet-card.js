    ...
    _applyOptimisticConsumption(qtyBags, season) {
      ...
      if (typeof this._currentStockBags === "number") {
        ...
        if (els.btnConso) els.btnConso.disabled = this._currentStockBags <= 0;
            ...
            if (typeof this._currentConsumedBags === "number") { this._currentConsumedBags += qtyBags; } var newConsumed = this._currentConsumedBags; if (cfg.show_stats && els.statConsomme && typeof newConsumed === "number") { els.statConsomme.textContent = fmt(newConsumed, 1) + " sac(s)"; if (els.statEnergie) { els.statEnergie.textContent = fmt(newConsumed * bagWeight * calorificValue, 1) + " kWh"; } } var todayISO2 = new Date().toISOString().slice(0, 10); var daysLogged; if (this._lastChartData && this._lastChartData.entries) { var seenDates = {}; for (var _i = 0; _i < this._lastChartData.entries.length; _i++) { var _e = this._lastChartData.entries[_i]; if (_e.type === "consumption") { seenDates[_e.date] = true; } } daysLogged = Object.keys(seenDates).length + (seenDates[todayISO2] ? 0 : 1); } else { daysLogged = (parseFloat(els.statJours && els.statJours.textContent) || 0) || 1; } if (cfg.show_stats && els.statJours) { els.statJours.textContent = String(daysLogged); } if (cfg.show_cost_stats && typeof newConsumed === "number" && typeof this._currentAvgPricePerBag === "number") { var avgPricePerBag = this._currentAvgPricePerBag; var costToDate = avgPricePerBag ? newConsumed * avgPricePerBag : 0; var costPerDay = daysLogged > 0 ? costToDate / daysLogged : 0; var costPerMonth = costPerDay * 30.44; if (els.statCoutAnnee) els.statCoutAnnee.textContent = fmt(costToDate, 2) + " €"; if (els.statCoutJour) els.statCoutJour.textContent = fmt(costPerDay, 2) + " €"; if (els.statCoutMois) els.statCoutMois.textContent = fmt(costPerMonth, 2) + " €"; } if (this._lastChartData && this._lastChartData.entries && typeof this._renderChart === "function") { var todayISO = new Date().toISOString().slice(0, 10); var newChartEntries = this._lastChartData.entries.slice(); newChartEntries.push({ type: "consumption", date: todayISO, qty_bags: qtyBags }); this._renderChart(newChartEntries, this._lastChartData.startMonth, this._lastChartData.avgPricePerBag); } if (els.historyList && !this._openEditRow) { this._renderHistory(newChartEntries); } }
